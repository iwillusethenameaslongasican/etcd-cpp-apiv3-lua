...
etcdv3::AsyncSetAction::AsyncSetAction(etcdv3::ActionParameters param, CompletionQueue* cq, unsigned long token,  bool create)
  : etcdv3::Action(param) 
{
  ...
  response_reader = parameters.kv_stub->AsyncTxn(&context, transaction.txn_request, cq);
  response_reader->Finish(&reply, &status, (void*)token);
}

etcd::Response etcdv3::AsyncSetAction::ParseResponse()
{
  ...
  etcd::Response resp = etcd::Response(txn_resp);          
  return resp;
  // fprintf(stdout, "action: %s | index: %d | value: %s\n", resp.action().c_str(), resp.index(), resp.value().as_string().c_str());
}
